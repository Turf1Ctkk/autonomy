...
int auto_landing;
int auto_planning;
ros::Time last_goal_publish_time(0);
double goal_publish_interval;
...
void Point_send(const ros::TimerEvent& event)
{
    ...
    // 确定当前目标点的索引
    int current_target = (counts == 0) ? 0 : (counts - 1);

    // 定时持续发送当前目标点（首次进入或者每隔 goal_publish_interval 秒重发）
    if ((now - last_goal_publish_time).toSec() >= goal_publish_interval)
    {
        goal.header.stamp = now;
        goal.pose.position.x = pytVector[current_target].x;
        goal.pose.position.y = pytVector[current_target].y;
        goal.pose.position.z = pytVector[current_target].z;
        point_pub.publish(goal);
        last_goal_publish_time = now;

        if(fligt_type == FLIGT_TYPE::PP_YAW_TIME){
            ROS_INFO("Publish pyt[%d]: [x:%f, y:%f, z:%f, yaw:%f, time:%f]", current_target, goal.pose.position.x, goal.pose.position.y, goal.pose.position.z, cmd_yaw.yaw, pytVector[current_target].time);
        }
        else if(fligt_type == FLIGT_TYPE::PP_TIME){
            ROS_INFO("Publish pyt[%d]: [x:%f, y:%f, z:%f, time:%f]", current_target, goal.pose.position.x, goal.pose.position.y, goal.pose.position.z, pytVector[current_target].time);
        }
        else if(fligt_type == FLIGT_TYPE::PP_YAW){
            ROS_INFO("Publish pyt[%d]: [x:%f, y:%f, z:%f, yaw:%f]", current_target, goal.pose.position.x, goal.pose.position.y, goal.pose.position.z, pytVector[current_target].yaw);
        }
        else{
            ROS_INFO("Publish pyt[%d]: [x:%f, y:%f, z:%f]", current_target, goal.pose.position.x, goal.pose.position.y, goal.pose.position.z);
        }
    }

    // 第一个点初始化：标记为已开始，等待下一个 tick 做距离判断
    if (counts == 0)
    {
        counts++;
        return;
    ...
    if (distance_ < next_distance && counts < pytVector.size())   //到达目标点并且还有下一个点
    {
        ...
        counts++;
        last_goal_publish_time = ros::Time(0);  // 强制下一个 tick 立即发送新的目标点

        ROS_INFO("Reached pyt[%d], advancing to pyt[%d]", counts_pre, counts_pre + 1);
    }
...
void startplan_cb(const geometry_msgs::PoseStamped::ConstPtr &msg)
{
    ...
    counts = 0;
    trigger = true;
    last_goal_publish_time = ros::Time(0);  // 重置发送计时，确保第一个点立即发出
...
int main(int argc, char** argv)
{
    ...
    // goal_publish_interval 为可选参数，默认 2.0 秒
    nh.param("/multipointplan/goal_publish_interval", goal_publish_interval, 2.0);
