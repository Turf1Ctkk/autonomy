...
class ImuProcess {
   public:
    ...
    bool IsImuNeedInit() const { return imu_need_init_; }
...
void ImuProcess::IMUInit(const common::MeasureGroup &meas, esekfom::esekf<state_ikfom, 12, input_ikfom> &kf_state,
    ...
    // gravity aligned with normalization
    Eigen::Vector3d gravity_normalized = mean_acc_.normalized();
    Eigen::Quaterniond rotation = Eigen::Quaterniond::FromTwoVectors(gravity_normalized, Eigen::Vector3d::UnitZ());
    mean_gyr_ = rotation * mean_gyr_;
    
    state_ikfom init_state = kf_state.get_x();
    init_state.rot = SO3(rotation.toRotationMatrix());
    init_state.grav = S2(Eigen::Vector3d(0, 0, -common::G_m_s2));
