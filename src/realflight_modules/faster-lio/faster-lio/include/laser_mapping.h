...
class LaserMapping {
   public:
    ...
    void ProcessIMUPropagation();
    void PropagateIMUOnce(state_ikfom &s, double dt, const common::V3D &acc, const common::V3D &gyro);
   ...
   private:
    ...
    void ApplyWorldTransform(Eigen::Vector3d &pos, Eigen::Quaterniond &quat);
    void ApplyWorldTransform(PointType *const pt);
    void ApplyWorldTransformPointCloudOnly(PointType *const pt);  // Only translation for point cloud
    void BuildWorldTransformMatrix();
   ...
   private:
    ...
    ///////////////////////// IMU propagation for high-frequency odometry /////////////////////////////////
    state_ikfom imu_propagate_state_;
    state_ikfom latest_ekf_state_;
    bool state_update_flg_ = false;
    bool imu_prop_enable_ = false;
    bool ekf_finish_once_ = false;
    std::deque<sensor_msgs::Imu> prop_imu_buffer_;
    sensor_msgs::Imu newest_imu_;
    double latest_ekf_time_ = 0.0;
    nav_msgs::Odometry imu_prop_odom_;
    ros::Publisher pub_imu_prop_odom_;
    std::string imu_prop_topic_ = "/lidar_slam/imu_propagate";
    std::mutex mtx_imu_prop_;
    double mean_acc_norm_ = 9.81;
    double last_prop_time_ = 0.0;

    // Low-pass filter for IMU propagation (only affects propagation path, not main LIO).
    bool filter_imu_en_ = false;
    bool imu_filter_initialized_ = false;
    double filter_alpha_ = 0.5;  // EMA coefficient: y[n] = alpha*x[n] + (1-alpha)*y[n-1]
    common::V3D acc_filtered_ = common::V3D::Zero();
    common::V3D gyro_filtered_ = common::V3D::Zero();
