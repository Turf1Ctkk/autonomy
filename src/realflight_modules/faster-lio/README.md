...
Faster-LIO 内部会按每条 IMU 数据传播完整 LIO 状态，并发布
`/lidar_slam/imu_propagate`，无需再启动独立的 `ekf` 高频里程计节点。
`mid360.yaml` 中的 `publish/imu_prop_use_filtered_imu` 可选择传播使用原始或低通滤波后的
IMU 副本，`publish/imu_prop_filter_alpha` 用于设置 EMA 系数。
