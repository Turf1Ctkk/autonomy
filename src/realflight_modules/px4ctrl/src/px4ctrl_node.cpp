...
int main(int argc, char *argv[])
{
    ...
    ros::Subscriber ommpc_trajectory_sub;
    ros::Subscriber ommpc_heartbeat_sub;
    if (param.pose_solver == 3)
    {
        ommpc_trajectory_sub =
            nh.subscribe<traj_utils::PolyTraj>("planning/trajectory",
                                               10,
                                               boost::bind(&Controller::feedOMMPCTrajectory,
                                                           &controller, _1),
                                               ros::VoidConstPtr(),
                                               ros::TransportHints().tcpNoDelay());
        ommpc_heartbeat_sub =
            nh.subscribe<std_msgs::Empty>("planning/heartbeat",
                                          20,
                                          boost::bind(&Controller::feedOMMPCHeartbeat,
                                                      &controller, _1));
        ROS_INFO("[PX4CTRL][OM-MPC] Waiting for native PolyTraj and planner heartbeat.");
    }
    ...
    ros::Publisher px4ctrl_state_pub = nh.advertise<quadrotor_msgs::Px4ctrlState>("/px4ctrl/state", 5);
    ...
    while (ros::ok())
    {
        ...
        switch (fsm.get_state()) {
            case PX4CtrlFSM::MANUAL_CTRL: state_msg.state = quadrotor_msgs::Px4ctrlState::MANUAL_CTRL; break;
            case PX4CtrlFSM::AUTO_HOVER:  state_msg.state = quadrotor_msgs::Px4ctrlState::AUTO_HOVER;  break;
            case PX4CtrlFSM::CMD_CTRL:    state_msg.state = quadrotor_msgs::Px4ctrlState::CMD_CTRL;    break;
            case PX4CtrlFSM::AUTO_TAKEOFF:state_msg.state = quadrotor_msgs::Px4ctrlState::AUTO_TAKEOFF;break;
            case PX4CtrlFSM::AUTO_LAND:   state_msg.state = quadrotor_msgs::Px4ctrlState::AUTO_LAND;   break;
            default: state_msg.state = 0; break;
        }
