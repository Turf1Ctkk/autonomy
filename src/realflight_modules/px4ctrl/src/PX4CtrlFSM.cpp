...
void PX4CtrlFSM::process()
{
	...
	bool publish_bodyrate_command = false;
	...
	case MANUAL_CTRL:
	{
		if (rc_data.enter_hover_mode) // Try to jump to AUTO_HOVER
		{
			...
			if (control_command_is_received(now_time))
			{
	...
	case AUTO_HOVER:
	{
		...
		else if (rc_data.is_command_mode && control_command_is_received(now_time))
		{
	...
	case CMD_CTRL:
	{
		const bool ommpc_offboard_lost =
			param.pose_solver == 3 && state_data.current_state.mode != "OFFBOARD";
		if (!rc_data.is_hover_mode || !odom_is_received(now_time) ||
			ommpc_offboard_lost)
		{
			if (param.pose_solver == 3)
				controller.invalidateOMMPCTrajectory(
					"CMD_CTRL aborted by RC, odometry loss, or OFFBOARD loss");
			state = MANUAL_CTRL;
			toggle_offboard_mode(false);
		...
		{
			state = AUTO_HOVER;
			bool hold_trajectory_end = false;
			if (param.pose_solver == 3 && param.ommpc.use_trajectory_ending_pos &&
				controller.ommpcTrajectoryFinished(now_time))
			{
				Eigen::Vector3d trajectory_end;
				if (controller.getOMMPCTrajectoryEndPosition(trajectory_end))
				{
					hover_pose.head<3>() = trajectory_end;
					hover_pose(3) = get_yaw_from_quaternion(odom_data.q);
					last_set_hover_pose_time = now_time;
					hold_trajectory_end = true;
				}
			}
			if (!hold_trajectory_end)
				set_hov_with_odom();
			if (param.pose_solver == 3)
			{
				if (!rc_data.is_command_mode)
					controller.invalidateOMMPCTrajectory("RC command mode disabled");
				else if (controller.ommpcHeartbeatTimedOut(now_time))
					controller.invalidateOMMPCTrajectory("planner heartbeat timed out");
			}
			des = get_hover_des();
	...
	else if (param.pose_solver == 3 && state == CMD_CTRL)
	{
		if (controller.update_alg3(odom_data, u, bat_data.volt, now_time))
		{
			publish_bodyrate_command = true;
			debug_msg = controller.debug;
			controller.fillOMMPCDebug(
				debug_msg, quadrotor_msgs::Px4ctrlDebug::OMMPC_ACTIVE);
			debug_msg.header.stamp = now_time;
			debug_pub.publish(debug_msg);
		}
		else
		{
			const std::string failure_reason = controller.ommpcLastError();
			controller.invalidateOMMPCTrajectory(
				std::string("MPC solve/reference failure: ") + failure_reason);
			state = AUTO_HOVER;
			set_hov_with_odom();
			des = get_hover_des();
			debug_msg = controller.update_alg0(des, odom_data, imu_data, u, bat_data.volt);
			controller.fillOMMPCDebug(
				debug_msg, quadrotor_msgs::Px4ctrlDebug::OMMPC_FALLBACK);
			debug_msg.header.stamp = now_time;
			debug_pub.publish(debug_msg);
			ROS_ERROR_STREAM("[px4ctrl][OM-MPC] Falling back to algorithm 0 hover: "
						 << failure_reason);
		}
	}
	else
	{
		const int active_solver = param.pose_solver == 3 ? 0 : param.pose_solver;
		...
		case 0:
			...
			controller.fillOMMPCDebug(
				debug_msg,
				param.pose_solver == 3
					? quadrotor_msgs::Px4ctrlDebug::OMMPC_STANDBY
					: quadrotor_msgs::Px4ctrlDebug::OMMPC_DISABLED);
			debug_msg.header.stamp = now_time;
			debug_pub.publish(debug_msg);
			break;
		case 1:
			...
			controller.fillOMMPCDebug(
				debug_msg, quadrotor_msgs::Px4ctrlDebug::OMMPC_DISABLED);
			debug_msg.header.stamp = now_time;
			debug_pub.publish(debug_msg);
			break;
...
Desired_State_t PX4CtrlFSM::get_takeoff_land_des(const double speed)
{
	...
	if (speed >= 0) {
		...
		if (tag_odom_data.recv_new_msg) {
			double dist_tolerance = 0.05;
			double dist_to_tag = (tag_odom_data.p.head(2) -  odom_data.p.head(2)).norm();
			des.p = takeoff_land.start_pose.head<3>();
			des.p.head(2) += (tag_odom_data.p.head(2) -  odom_data.p.head(2)) / dist_to_tag * 0.2 / param.ctrl_freq_max;
			des.v = Eigen::Vector3d(0, 0, 0);
			des.v.head(2) = (tag_odom_data.p.head(2) -  odom_data.p.head(2)) / dist_to_tag * 0.2;
			static bool is_landing = false;
			if (dist_to_tag > dist_tolerance && !is_landing) {
				takeoff_land.start_pose.head(3) = des.p;
...
bool PX4CtrlFSM::control_command_is_received(const ros::Time &now_time)
{
	if (param.pose_solver == 3)
		return controller.ommpcTrajectoryReady(now_time);
	return cmd_is_received(now_time);
}
