...
Controller::Controller(Parameter_t &param_) : param(param_)
{
  ...
  {
    ommpc_controller_.reset(new px4ctrl_ommpc::OnManifoldMPC());
    ommpc_initialized_ = ommpc_controller_->initialize(param);
    if (!ommpc_initialized_)
    {
      ROS_ERROR_STREAM("[px4ctrl][OM-MPC] Initialization failed: "
                       << ommpc_controller_->lastError());
    }
  }
}
...
/************* Algorithm3: on-manifold MPC, start ***************/
bool Controller::update_alg3(
    const Odom_Data_t &odom,
    Controller_Output_t &u,
    double voltage,
    const ros::Time &now)
{
  ommpc_raw_throttle_ = 0.0;
  ommpc_mapped_throttle_ = 0.0;
  ommpc_throttle_saturated_ = false;
  if (!ommpc_initialized_ || !ommpc_controller_)
  {
    ROS_ERROR_THROTTLE(1.0, "[px4ctrl][OM-MPC] Controller is unavailable.");
    return false;
  }

  px4ctrl_ommpc::ControlOutput output;
  if (!ommpc_controller_->update(odom.p, odom.v, odom.q, now, output))
  {
    ROS_ERROR_STREAM_THROTTLE(1.0, "[px4ctrl][OM-MPC] "
                                      << ommpc_controller_->lastError());
    return false;
  }

  u.bodyrates = output.bodyrates;
  ommpc_raw_throttle_ = mapCollectiveAccelerationToThrustSignal(
      output.collective_acceleration, voltage, param);
  u.thrust = ommpc_raw_throttle_;
  if (!u.bodyrates.allFinite() || !std::isfinite(u.thrust))
  {
    ROS_ERROR("[px4ctrl][OM-MPC] Non-finite command after thrust mapping.");
    return false;
  }

  if (u.thrust < 0.0 || u.thrust > 1.0)
  {
    ommpc_throttle_saturated_ = true;
    ROS_WARN_THROTTLE(1.0,
                      "[px4ctrl][OM-MPC] Normalized thrust %.3f saturated to [0, 1].",
                      u.thrust);
    u.thrust = std::max(0.0, std::min(1.0, u.thrust));
  }
  ommpc_mapped_throttle_ = u.thrust;

  const px4ctrl_ommpc::DebugOutput &ommpc_debug =
      ommpc_controller_->debugOutput();
  if (ommpc_debug.reference_valid)
  {
    debug.des_v_x = ommpc_debug.reference_velocity.x();
    debug.des_v_y = ommpc_debug.reference_velocity.y();
    debug.des_v_z = ommpc_debug.reference_velocity.z();
    debug.des_a_x = ommpc_debug.reference_acceleration.x();
    debug.des_a_y = ommpc_debug.reference_acceleration.y();
    debug.des_a_z = ommpc_debug.reference_acceleration.z();
    debug.des_q_w = ommpc_debug.reference_attitude.w();
    debug.des_q_x = ommpc_debug.reference_attitude.x();
    debug.des_q_y = ommpc_debug.reference_attitude.y();
    debug.des_q_z = ommpc_debug.reference_attitude.z();
  }
  debug.des_thr = output.collective_acceleration;
  debug.fb_rate_x = output.bodyrates.x();
  debug.fb_rate_y = output.bodyrates.y();
  debug.fb_rate_z = output.bodyrates.z();
  const double attitude_error_angle = ommpc_debug.attitude_error.norm();
  Eigen::Vector3d attitude_error_axis = Eigen::Vector3d::Zero();
  if (attitude_error_angle > 1.0e-9)
    attitude_error_axis = ommpc_debug.attitude_error / attitude_error_angle;
  debug.exec_err_axisang_x = attitude_error_axis.x();
  debug.exec_err_axisang_y = attitude_error_axis.y();
  debug.exec_err_axisang_z = attitude_error_axis.z();
  debug.exec_err_axisang_ang = attitude_error_angle;
  debug.hover_percentage = param.gra / thr2acc;
  debug.thr_scale_compensate = thr_scale_compensate;
  debug.voltage = voltage;

  // Timestamp the command when it is actually generated. This preserves the
  // 35--45 ms command/IMU alignment used by px4ctrl's stable thrust estimator.
  timed_thrust.push(std::make_pair(ros::Time::now(), u.thrust));
  while (timed_thrust.size() > 100)
    timed_thrust.pop();

  ROS_INFO_THROTTLE(2.0, "[px4ctrl][OM-MPC] Solver latency: %.3f ms",
                    ommpc_controller_->timingMilliseconds());
  return true;
}

void Controller::feedOMMPCTrajectory(const traj_utils::PolyTrajConstPtr &msg)
{
  if (ommpc_controller_)
    ommpc_controller_->feedTrajectory(msg);
}

void Controller::feedOMMPCHeartbeat(const std_msgs::EmptyConstPtr &msg)
{
  if (ommpc_controller_)
    ommpc_controller_->feedHeartbeat(msg);
}

bool Controller::ommpcTrajectoryReady(const ros::Time &now) const
{
  return ommpc_controller_ && ommpc_controller_->trajectoryReady(now);
}

bool Controller::ommpcTrajectoryFinished(const ros::Time &now) const
{
  return ommpc_controller_ && ommpc_controller_->trajectoryFinished(now);
}

bool Controller::ommpcHeartbeatTimedOut(const ros::Time &now) const
{
  return ommpc_controller_ && ommpc_controller_->heartbeatTimedOut(now);
}

bool Controller::getOMMPCTrajectoryEndPosition(Eigen::Vector3d &position) const
{
  return ommpc_controller_ && ommpc_controller_->getTrajectoryEndPosition(position);
}

void Controller::invalidateOMMPCTrajectory(const std::string &reason)
{
  if (ommpc_controller_)
    ommpc_controller_->invalidateTrajectory(reason);
}

const std::string &Controller::ommpcLastError() const
{
  static const std::string unavailable("OM-MPC is unavailable");
  return ommpc_controller_ ? ommpc_controller_->lastError() : unavailable;
}

void Controller::fillOMMPCDebug(quadrotor_msgs::Px4ctrlDebug &message,
                                const uint8_t status) const
{
  message.ommpc_status = status;
  const bool include_latest =
      status == quadrotor_msgs::Px4ctrlDebug::OMMPC_ACTIVE ||
      status == quadrotor_msgs::Px4ctrlDebug::OMMPC_FALLBACK;

  px4ctrl_ommpc::DebugOutput data;
  if (include_latest && ommpc_controller_)
    data = ommpc_controller_->debugOutput();

  message.ommpc_reference_valid = data.reference_valid;
  message.ommpc_solver_success = data.solver_success;
  message.ommpc_throttle_saturated =
      include_latest && ommpc_throttle_saturated_;
  message.ommpc_traj_id = data.trajectory_id;
  message.ommpc_traj_time = data.trajectory_time;
  message.ommpc_solve_time_ms = data.solve_time_ms;
  message.ommpc_optimal_cost = data.optimal_cost;

  message.ommpc_ref_p_x = data.reference_position.x();
  message.ommpc_ref_p_y = data.reference_position.y();
  message.ommpc_ref_p_z = data.reference_position.z();
  message.ommpc_ref_v_x = data.reference_velocity.x();
  message.ommpc_ref_v_y = data.reference_velocity.y();
  message.ommpc_ref_v_z = data.reference_velocity.z();
  message.ommpc_ref_a_x = data.reference_acceleration.x();
  message.ommpc_ref_a_y = data.reference_acceleration.y();
  message.ommpc_ref_a_z = data.reference_acceleration.z();
  message.ommpc_ref_q_w = data.reference_valid ? data.reference_attitude.w() : 0.0;
  message.ommpc_ref_q_x = data.reference_valid ? data.reference_attitude.x() : 0.0;
  message.ommpc_ref_q_y = data.reference_valid ? data.reference_attitude.y() : 0.0;
  message.ommpc_ref_q_z = data.reference_valid ? data.reference_attitude.z() : 0.0;

  message.ommpc_err_p_x = data.position_error.x();
  message.ommpc_err_p_y = data.position_error.y();
  message.ommpc_err_p_z = data.position_error.z();
  message.ommpc_err_v_x = data.velocity_error.x();
  message.ommpc_err_v_y = data.velocity_error.y();
  message.ommpc_err_v_z = data.velocity_error.z();
  message.ommpc_err_att_x = data.attitude_error.x();
  message.ommpc_err_att_y = data.attitude_error.y();
  message.ommpc_err_att_z = data.attitude_error.z();

  message.ommpc_ref_collective_acc = data.reference_collective_acceleration;
  message.ommpc_cmd_collective_acc = data.commanded_collective_acceleration;
  message.ommpc_cmd_throttle_raw = include_latest ? ommpc_raw_throttle_ : 0.0;
  message.ommpc_cmd_throttle = include_latest ? ommpc_mapped_throttle_ : 0.0;
  message.ommpc_ref_rate_x = data.reference_bodyrates.x();
  message.ommpc_ref_rate_y = data.reference_bodyrates.y();
  message.ommpc_ref_rate_z = data.reference_bodyrates.z();
  message.ommpc_cmd_rate_x = data.commanded_bodyrates.x();
  message.ommpc_cmd_rate_y = data.commanded_bodyrates.y();
  message.ommpc_cmd_rate_z = data.commanded_bodyrates.z();
  message.ommpc_thr2acc = include_latest ? thr2acc : 0.0;
  message.ommpc_thrust_scale_compensate =
      include_latest ? thr_scale_compensate : 0.0;
}
/************* Algorithm3: on-manifold MPC, end ***************/
...
double Controller::mapCollectiveAccelerationToThrustSignal(
    const double collective_acceleration,
    double voltage,
    const Parameter_t &param) const
{
  const double bounded_acceleration = std::max(0.0, collective_acceleration);
  if (param.thr_map.accurate_thrust_model)
  {
    return thr_scale_compensate *
           AccurateThrustAccMapping(bounded_acceleration, voltage, param);
  }
  return bounded_acceleration / thr2acc;
}
