...
public:
	...
	/* Algorithm3: on-manifold MPC, active only for PolyTraj in CMD_CTRL. */
	bool update_alg3(
		const Odom_Data_t &odom,
		Controller_Output_t &u,
		double voltage,
	...
	void feedOMMPCTrajectory(const traj_utils::PolyTrajConstPtr &msg);
	void feedOMMPCHeartbeat(const std_msgs::EmptyConstPtr &msg);
	bool ommpcTrajectoryReady(const ros::Time &now) const;
	bool ommpcTrajectoryFinished(const ros::Time &now) const;
	bool ommpcHeartbeatTimedOut(const ros::Time &now) const;
	bool getOMMPCTrajectoryEndPosition(Eigen::Vector3d &position) const;
	void invalidateOMMPCTrajectory(const std::string &reason);
	const std::string &ommpcLastError() const;
	void fillOMMPCDebug(quadrotor_msgs::Px4ctrlDebug &message,
	...
	double mapCollectiveAccelerationToThrustSignal(
...
private:
	std::unique_ptr<px4ctrl_ommpc::OnManifoldMPC> ommpc_controller_;
	bool ommpc_initialized_{false};
	double ommpc_raw_throttle_{0.0};
	double ommpc_mapped_throttle_{0.0};
	bool ommpc_throttle_saturated_{false};
